...
def test_parses_all_sample_task_types_and_reward() -> None:
    ...
    assert {TaskType.CHECKMARK, TaskType.KILL, TaskType.ITEM, TaskType.LOCATION, TaskType.ADVANCEMENT} <= task_types
    assert RewardType.ITEM in reward_types

def test_preserves_unknown_task_payload() -> None:
    project = FTBQuestParser().load(FIXTURE)
    checkmark = next(task for quest in project.quests for task in quest.tasks if task.type is TaskType.CHECKMARK)
