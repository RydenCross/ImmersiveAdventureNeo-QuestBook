...
def run_audit_performance_contract(
    ...
    *,
    ...
    duplicate_executions = tuple(sorted({name for name in executions if executions.count(name) > 1}))
    missing_timings = tuple(sorted(set(names) - {item.name for item in timings})) if probes is None else ()
