...
class DocumentationAudit:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "markdown_file_count": len(self.markdown_files),
            "markdown_files": list(self.markdown_files),
            "documented_command_count": len(EXPECTED_COMMANDS) - len(self.missing_command_docs),
            "expected_command_count": len(EXPECTED_COMMANDS),
            "missing_command_docs": list(self.missing_command_docs),
            "broken_links": list(self.broken_links),
            "empty_documents": list(self.empty_documents),
        }
    ...
    def format(self) -> str:
        lines = [
            f"Documentation contract audit: {'PASS' if self.is_clean else 'FAIL'}",
            f"Markdown files: {len(self.markdown_files)}.",
            f"Documented commands: {len(EXPECTED_COMMANDS) - len(self.missing_command_docs)}/{len(EXPECTED_COMMANDS)}.",
            f"Broken links: {len(self.broken_links)}.",
            f"Empty documents: {len(self.empty_documents)}.",
        ]
...
def run_documentation_audit(
    readme: Path = DEFAULT_README, docs: Path = DEFAULT_DOCS
) -> DocumentationAudit:
    ...
    missing = tuple(
        sorted(
            ...
            if f"python -m generator {command}" not in combined
            and f"immersive-adventure-quests {command}" not in combined
        )
    )
    broken = []
    for source, text in texts.items():
        for target in _MARKDOWN_LINK.findall(text):
            ...
            if (
                ...
                and not (source.parent / clean).resolve().exists()
            ):
    ...
    return DocumentationAudit(
        tuple(path.as_posix() for path in files), missing, tuple(sorted(set(broken))), empty
    )
